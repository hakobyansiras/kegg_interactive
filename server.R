...
psf_signal_calculator_and_coloring <- function(entrez_fc, pathway, pathway_name, update_mod = FALSE, old_graph = NULL, no_color_mode = F) {
  ...
  exp_values <- exp_values[order(exp_values)]
  
  if(no_color_mode) {
    exp_colors <- NULL
  } else {
    exp_colors <- c(pal1(10)[cut(exp_values[which(exp_values <= 1)],10)], 
                    pal2(10)[cut(exp_values[which(exp_values > 1)],10)])
    ...
    exp_colors = data.frame(node_id = names(exp_values)[c(which(exp_values <= 1), which(exp_values > 1))], 
                            col = exp_colors,
  ...
  signal_values <- signal_values[order(signal_values)]
  
  psf_colors <- c(pal1(10)[cut(signal_values[which(signal_values <= 1)],10)], 
                  pal2(10)[cut(signal_values[which(signal_values > 1)],10)])
  ...
  psf_colors = data.frame(node_id = names(signal_values)[c(which(signal_values <= 1), which(signal_values > 1))], 
                          col = psf_colors,
                          text_col = unname(sapply(psf_colors, function(x) {c( "black", "white")[  1+(sum( col2rgb(x) *c(299, 587,114))/1000 < 123) ]})),
                          stringsAsFactors = F
  )
  
  return(list(psf_colors = psf_colors, exp_colors = exp_colors, exp_values = exp_values, signal_values = signal_values, psf_graph = psf_graph))
...
shinyServer(function(input, output, session) {
  ...
  v <- reactiveValues(image_file = kegg_node_mapper(group_graphics = group_graphics[["Chemokine_signaling_pathway"]], kegg_pathway_graphics = graphical_data_generator(chemokine_pathway_data$pathway), pathway_name = "Chemokine_signaling_pathway", pathway_image = chemokine_pathway_data$image, show_changes = F) %>% 
                      ...
                      allow_graph_param_update = FALSE
  )
  ...
  observeEvent(input$psf_run, {
    ...
    v$col_legend_title <- "Signal value"
